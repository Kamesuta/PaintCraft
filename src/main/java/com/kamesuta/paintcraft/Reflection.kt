...
object Reflection {
    fun getMapCache(mapView: MapView): ByteArray {
        val colors: ByteArray? = try {
            val worldMap = Accessor.getField(mapView, "renderCache") as HashMap<*, *>
            val renderCache = worldMap[null]
                ?: worldMap.values.firstOrNull()
            if (renderCache != null) {
                Accessor.getField(renderCache, "buffer") as ByteArray
            } else {
                null
            }
            ...
            try {
                ...
                throw NoSuchFieldException(
                    String.format(
                        ...
                        fieldName, obj.javaClass.name, Arrays.asList(*obj.javaClass.declaredFields)
                    )
                )
            }
            return field[obj]
            ...
            try {
                ...
                throw NoSuchMethodException(
                    String.format(
                        ...
                        methodName, obj.javaClass.name, Arrays.asList(*obj.javaClass.declaredMethods)
                    )
                )
            }
        ...
        fun invokeStaticMethod(className: String, methodName: String?, vararg params: Any): Any {
            ...
            try {
                ...
                throw Exception(
                    String.format(
                        "Method '%s' could not be found in '%s'. Methods found: [%s]",
                        methodName, obj.name, Arrays.asList(*obj.methods)
                    ), e
                )
            }
